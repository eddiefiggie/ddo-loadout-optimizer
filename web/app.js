...
if (typeof window !== "undefined" && typeof document !== "undefined") {
  ...
  loadDataset()
    .then((raw) => {
      ...
      // #795 — the Essence Crafting placement table, published beside the dataset
      // and read by the player-authored item bench. Set here, at the one place the
      // dataset arrives, so a caller that forgets to pass it on `ctx` still
      // validates against the real table instead of an empty one — which would
      // refuse every slot with "not a slot this build knows". `custom-items.js`
      // prefers the ctx copy and never reaches for this first, so the Node tests
      // and the browser read the same data.
      window.__essencePlacements = dataset.essence_placements || null;
