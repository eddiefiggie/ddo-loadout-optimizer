...
/** #721 — the owned-pool pin disclosure. The fact is computed where the pool is
 *  (wizard.js `pinnedUnownedNames`, which owns the ownership predicate) and read
 *  here, rather than re-derived from the result — results.js has no view of the
 *  player's import. */
function pinnedUnownedNoticeEntries(query) {
...
const NOTICES = [
  ...
  // #663 — ACTIONABLE, unlike the #573 disclosure directly above it, and the
  // difference is worth stating because the two look alike. There we do not have
  // the number and neither does the wiki, so the player can only be told; here the
  // ceiling IS known (40) and setting a Max of 40 fully resolves it. What we cannot
  // decide FOR them is whether 40 is right for their character — Sneak, armor check
  // penalty and falling damage each move the useful target — so the notice names
  // the ceiling and the three escapes, and leaves the choice on the priorities step.
  { name: "jumpSoftCapNotice", id: "jump-soft-cap", title: "JUMP ABOVE 40",
  ...
  // #850 — QUALIFYING, like #683 directly below and for the same reason: the
  // player CAN press something (rank the sibling name too) but the wiki rules
  // the two names different effects, so the app does not merge them and does
  // not present ranking the other as "the fix" — it says what the upper tier
  // carries and lets the player decide. The jump is offered because the choice,
  // if they want it, lives on the priorities step.
  { name: "tierRenameNotice", id: "tier-rename", title: "RENAMED ON UPGRADE",
    subject: "tier rename", cls: NOTICE_QUALIFYING,
    jump: { label: "Edit priorities \u2192", step: "priorities", anchor: null },
    render: (c) => tierRenameNotice(c.query, c.result) },
...
const NOTICE_ENTRY_JUMPS = {
  ...
  // #721 — the same route as its sibling: both are a pin overriding a pool
  // choice, and the control that undoes either is the pin list.
  "pinned-not-owned": { label: "Review pins →", step: "pool", anchor: null },
...
/** #663 — the Jump soft-cap disclosure. Same contract as the notice above: reads
...
function jumpSoftCapNotice(query, result) {
  const line = (Proj && Proj.jumpSoftCapLine)
...
/** #850 — a ranked presence effect an item family renames on upgrade. Reads the
 *  SHARED lines from projection, never a second wording. */
function tierRenameNotice(query, result) {
  const lines = (Proj && Proj.tierRenameNoticeLines)
    ? Proj.tierRenameNoticeLines({ snapshot: result, query }) : [];
  if (!lines || !lines.length) return "";
  return lines.map((l) =>
    `<p class="scope-note tier-rename-note" role="status">${esc(l)}</p>`).join("");
}
