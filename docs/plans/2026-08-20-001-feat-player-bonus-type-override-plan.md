...
- The provenance key name for the original type, and where the override overlay sits relative to the existing load-time normalizers.
- Where the manager view lives in the wizard and tab structure.
- Whether re-confirming a suspended override rewrites its recorded type or replaces the override.
- Whether the three added vocabulary entries need any guard keeping the shared list and the equivalence map in step as upstream changes.
- Whether an override on an affix a maintainer type-correction also targets is refused, or the correction wins. Both mechanisms write the same field and only one has a stale guard today.
- Whether "the solve used at least one override" in R14 counts an override that is present but suspended, or present but whose affix was not selected.
- Whether the correction report is its own surface or an extension of the six exports. `Projection.project(rec)` is loadout-scoped, so it cannot host a report covering a Browse-created or pool-keyed override; the manager view or a constraint-header-style block is the viable carrier, following the declared-credits precedent.
- Which reported cases in `data/bug_reports.txt` the 20,613 eligible affixes actually reach. The coverage claim needs tracing against the real reports rather than resting on one example — #259 is cause 1, already fixed globally, so it does not demonstrate in-scope coverage.
- Whether sharing the vocabulary list means `Orb`, `Sneak Attack`, and `Determination` also appear in the declared-credit selector, which renders from the same constant.
- How the recorded type and drift detection work for a pool-keyed override, whose pool entry can be retyped upstream independently of any host.
