...
- R5a. `Item:Belt of the Ram` is corrected from its false `unsourced` reading to `{{Speed|15}}`, provenance `defaulted`, movement 15. The wiki states `Speed +15%` with a tooltip of "+15% enhancement bonus to movement speed, 5% bonus to attack speed" — a placeholder 5%, so no alacrity. Solver output does not change: the `unsourced` branch already keeps gear-planner's `Speed: 15` and renames it, so the item is not losing its movement bonus today. The correction buys coverage and honesty — the entry gains a real invocation so it can carry a snapshot, and the shard stops asserting "no Striding/Speed template found" about a page that plainly has one. After it lands no entry lacks an invocation, and the guard fails if any entry does.
