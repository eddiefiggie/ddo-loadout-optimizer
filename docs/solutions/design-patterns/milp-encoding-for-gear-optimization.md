...
**Known refinements (open at time of writing):** a code-review pass flagged two edge issues in this same code — a capped target with *zero* eligible sources reports the cap instead of 0 (the `d <= raw` constraint is skipped when `raw` is empty), and multiple weapon *types* are modeled as independent slots (the solver could equip several weapons at once). Both are follow-ups; the encoding patterns above are unaffected. Set-bonus and augment optimization are also deferred (their bonuses are still free text — see `parsing-ddo-wiki-affix-text.md`).
