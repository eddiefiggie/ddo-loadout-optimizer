...
- `web/solver.js` `readSolution` — all eleven guards side by side: jokerPlaced's `realShort` (set active AND real pieces short), membershipPlaced's `activeSetNames`, setAugmentsPlaced's `setAugCount >= pieces_required` (PR #318), the seven craft families' shared `fired.has(var)` (fired-contribution membership, #319), and augmentsPlaced's `firedAugIds.has(variant_id)` (identity join through `placeMeta`, #319).
