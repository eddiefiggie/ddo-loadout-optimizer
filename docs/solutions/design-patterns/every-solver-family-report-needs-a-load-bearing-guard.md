...
**The catalog closed with #319** (the audit this doc's third instance triggered): the eight remaining families — ordinary augments, Dino, Nearly Completed, roll groups, Viktranium, seal, Thunder-Forged, Green Steel — all report through one uniform guard now. The audit found they share a single load-bearing shape: every one delivers value exclusively through gated `z` contribution vars, so a placement is load-bearing iff some `z` it gates fired. The seven craft families check membership directly, and ordinary augments join through `placeMeta` by `variant_id` because their value gates ride on the `pu` placement-identity var while the reported entries are the per-color `p` vars (`Σp = pu` makes the join sound). The audit also overturned this doc's original two-path framing: the optimum path never minimizes the seven craft families either (the tie-break covers only jokers/memberships/set-augment copies, the settle stage only ordinary augments), so floats were reachable on BOTH paths for those seven — the guard is not an alternatives-only concern.

**#322 refined fired to fired-and-VISIBLE.** A fired contribution can still change nothing the player sees, in exactly two shapes: (a) **credit substitution** — the z seats in a bucket carrying a declared-credit floor at a value at or below that floor, duplicating a bonus the player already declared; (b) **cap clamping** — every tracked stat the z feeds is capped and the placement's contributions are jointly slack under the cap. `firedGateSet` became `visibleGateSet` (web/solver.js): credit-substitutable z's drop first, then cap clamping is judged **per placement, per stat, as a SUM test** (`raw(s) − Σ(placement's surviving contributions to s) ≥ cap(s)`), never per-contribution — a per-contribution test would hide a placement whose contributions jointly hold a stat at its cap — and **set-consistently across placements**: gates are judged greedily in a deterministic (sorted) order, each gate ruled hidden deducting its sums from an adjusted raw before the next is judged, so raw minus ALL hidden contributions still meets every cap, the displayed capped totals stay reachable from the reported placements, and two placements jointly holding a stat at cap can never hide each other (the sorted order makes which one survives stable). The stats a z feeds are `targetList ∪ keys(cappedStats)` via `bucketCountsFor` (never the bucket's own stat alone — a clamped cross-add source still raises uncapped sibling totals), and an uncapped-fed bucket short-circuits to visible, so behavior outside the two shapes is unchanged. The same predicate filters `breakdownByTarget`, so attribution and every export (projection.js reads `result.breakdown` directly) agree with the placement arrays.

## Guidance

When adding a new placement-var family whose placements `readSolution` reports:

1. **Define load-bearing for THAT family, and encode it as a report guard.** Ask: under what condition does this placement actually change the player's outcome? Set is active and real pieces fall short (joker); set is active (membership); enough copies to fire the tier (set augments); a gated contribution fired AND is visible in the displayed outcome (the z-gated families — firing alone is not sufficient, because caps and declared-credit floors are two shapes where a fired contribution changes nothing visible, #322). The guard lives in `readSolution`, next to the family's report loop, so it holds on every solve path.
...
3. **The guard test is deterministic at the report layer even when the float itself is nondeterministic.** You cannot write a test that reliably makes HiGHS float a var — but you can feed `readSolution` a primal where the var is 1 and its load-bearing condition is false, and assert the report omits it. Test the guard, not the float. Since #319 this is a first-class seam: `readSolution` is exported for tests only, and the synthetic primal is just `{ Columns: { <var>: { Primal: 1 } } }` (absent names read 0). Pair each omission test with a positive arm (condition true → reported) so an over-strict guard cannot pass vacuously, and mutation-check each guard: disable its condition and confirm that family's test — and only that family's — goes red.
...
- `web/solver.js` `readSolution` — all eleven guards side by side: jokerPlaced's `realShort` (set active AND real pieces short), membershipPlaced's `activeSetNames`, setAugmentsPlaced's `setAugCount >= pieces_required` (PR #318), the seven craft families' shared `fired.has(var)` over `visibleGateSet` (fired-and-visible membership, #319/#322), and augmentsPlaced's `firedAugIds.has(variant_id)` (identity join through `placeMeta`, #319). The #322 seam arms in `tests/solver.test.js` (capped omission/positive, multi-affix, joint-saturation sum test, breakdown, credit omission/positive) are the pattern for testing a visibility condition.
...
- Sibling concerns on the same PR, at other layers: `add-a-solver-preference-as-a-pinned-post-stage.md` (pin granularity on the optimum-path stage chain), `canonicalization-guards-need-a-trial-reassignment-and-identity-check.md` (projection-layer trial-assignment guard), and `widening-eligibility-must-re-derive-derived-constraints.md` (constraint-emission coefficients) — five standing rules from one PR: these four runtime layers plus the test-fixture layer (`docs/solutions/conventions/stamp-shared-fixtures-for-new-required-fields.md`).
- The test-side complement from the #319 completion (PR #323): `docs/solutions/conventions/assert-non-vacuity-for-every-surface-in-a-loop-test.md` — a loop test pinning all eight guarded families is vacuous for any family its fixture never mints; the guard-catalog work is only as proven as the fixture behind it.
