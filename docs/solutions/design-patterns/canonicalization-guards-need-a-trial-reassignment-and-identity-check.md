...
- The other review-confirmed defects from the same PR, at different layers: `add-a-solver-preference-as-a-pinned-post-stage.md` (solver post-stage pin granularity), `every-solver-family-report-needs-a-load-bearing-guard.md` (report-layer float guards), and `widening-eligibility-must-re-derive-derived-constraints.md` (constraint-emission coefficients). Five standing rules from one PR: these four runtime layers plus the test-fixture layer (`docs/solutions/conventions/stamp-shared-fixtures-for-new-required-fields.md`).
