...
applies_when:
  ...
  - "A guard's predicate matches on a field whose spelling differs between two representations"
...
**3. When a field has two spellings, say so at both sites.** The mismatch was not a typo
— both spellings are correct in their own context. Leave a comment naming the other one
...
- Any predicate matching on a field whose representation differs between a raw source and a
  normalized one. This project has at least one such pair (`Offhand` / `Off Hand`) and the
