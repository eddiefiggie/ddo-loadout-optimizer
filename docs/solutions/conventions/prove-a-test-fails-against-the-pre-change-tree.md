...
A seventh instance surfaced in PRs #354/#355 (2026-08-17), and it is the **client-side twin of the sixth**. Entry six was a path the *test runner* never invokes (a file outside the glob). Entry seven is a path the *app* never invokes: a results-panel disclosure was verified by calling `buildModel` directly in Node and worked, while in the shipped app it rendered nowhere — `web/query.js`, the file that had been wired, is not in `web/index.html`'s script list, and the live solve path is `web/wizard.js`'s two `buildModel` calls, both still passing the old argument. A sibling case in the same change verified a marker helper directly while the real `affixEntries -> presenceMarker` join reached 75 of 1,656 target chips. Neither gate above asks *does the app invoke this path*. The full case is in [browser-verify-against-real-data-not-just-unit-tests](../developer-experience/browser-verify-against-real-data-not-just-unit-tests.md).

**A note on the gates' shared presumption.** All of the above — the base-tree run, the mutation check, and the per-surface non-vacuity assertion — presume that *something executes the path*: the runner for a test, the app for a feature. That presumption is cheap to verify and, until #343, was never stated. It has now failed twice on two different surfaces — the runner (entry six) and the app (entry seven) — so verify both: read the CI command and confirm the path matches, and read the loader and confirm the app invokes what you changed.

**Where a source-text assertion is load-bearing rather than suspect.** This doc is otherwise wary of regexes over source. The live-call-site wiring guard is the canonical exception, and it falls under the existing "reach for the regex only where extraction genuinely is not available" clause: no unit test can observe whether the *app's own* call passes the right thing, because a unit test supplies the arguments itself. `tests/wizard.test.js` holds three such guards: one over the `buildModel` call sites (asserting the *count*, so a third cannot appear unguarded, plus a negative assertion that the superseded bare-Set form does not survive), one pinning that `initBrowse` receives the picker vocabulary, and one pinning that `index.html` does not load `query.js`.

Their prove-red status is not uniform, which this doc should say plainly: the `initBrowse` guard was proven red by reverting its fix; the call-site guard is red by construction, because its negative assertion names the superseded form verbatim; and the `index.html` guard is a deliberate nothing-changed pin — the exemption this doc already grants — since the app never loaded that file and there is no fix to revert.
