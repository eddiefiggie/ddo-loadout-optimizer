...
**Current build:** 08152026.3 — the live site's footer shows the deployed value. `tests/test_build_stamp.py` fails the build when this line drifts from `web/app.js`, so it cannot go stale silently.
...
>
> **Latest work (PR #317 + #318, merged & deployed — closes #312/#316):** the set-augment placement arc. PR #317: one set-bonus identity per item (copies and awakenings never stack; per-host mutual exclusion). PR #318: set-augment copies place in **any standard color slot** via the defs-stamped `fits_slots` matrix — per-copy color variables sharing real capacity, a **Colorless-first pinned post-stage** (third stage in the chain, per the #206 pattern with new pin-granularity rules), trial-assignment-guarded read-time canonicalization in projection, slot attribution on every chip and export, and a coverage-note disclosure read off the defs. The planned Moon/Sun deferral **resolved same-day** by the one-hop wiki sweep: the Augment Slot page rules Lunar/Solar slots outside the standard color system entirely, so the seven-color matrix is wiki-ruled exact (dated ruling in `docs/wiki-evidence/augment-sets.md`; no follow-up issue needed). Learnings banked (PR #320): five standing rules from the one PR — pin granularity, report-layer load-bearing guards (third instance), canonicalization-guard rules, derived-constraint re-derivation, and fixture stamping — plus sweep-before-deferring, the guard-falsification interception case study, and the meta-keying mechanism note. Follow-up filed: #319 (audit the remaining unguarded `*Placed` families for alternatives floats).
>
