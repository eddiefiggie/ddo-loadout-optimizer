...
**Current build:** 09172026.2 — the live site's footer shows the deployed value. `tests/test_build_stamp.py` fails the build when this line drifts from `web/app.js`, so it cannot go stale silently.
...
| Filigrees | **Not modelled, by decision** (#196, 2026-09-17) — a non-goal, not a gap. No source carries filigree data: the seed tree holds one prose line about slot counts and nothing else. #412 put them out of scope for the owned-inventory path, and the app tells players so. |
...
**The one exception is the one you make yourself, and it is labelled.** You can describe an item the catalog has never heard of — the Cannith-crafted ring you are weighing up, the Essence-crafted dagger you already made — and the solver will use it. Those numbers are the only ones in the tool that did not come from the wiki, so they never pass as if they had: the item is named `(yours)` on the paperdoll, in the pin list and in all six exports, and a result that uses one says outright that it is only as right as what you typed. The same rule already applied to the two smaller versions of this — a declared "I already have this" bonus, and a bonus type you corrected by hand.
...
>
> **Genuinely open:** [all open issues](https://github.com/eddiefiggie/ddo-loadout-optimizer/issues) — GitHub Issues is the single source of truth, so do not maintain a second list here. Self-deferred work carries the [`backlog`](https://github.com/eddiefiggie/ddo-loadout-optimizer/issues?q=is%3Aissue+is%3Aopen+label%3Abacklog) label; user-reported work is `bug`/`enhancement`. **5 open, of which one is not `backlog`** — #746; everything else is self-deferred. Every open issue's premise was re-measured against the built dataset on 2026-09-15 and every one still holds. **Next up, in the order judged 2026-09-15:** **the wiki window** — #746 first, then #591 half B, then #769 — because all three are blocked on the same thing, a same-origin ddowiki tab that no build container has, and nothing else in the queue needs one. #763 made the first two paste-and-run (`scripts/browser/`); #769 is one page read, added to that plan as a third snippet by #777. Measured today: #746's four names still carry **129 / 112 / 69 / 23** items (`Ghostbane` / `Ghostly` / `Ghost Touch` / `Ethereal`) and the shipped picker hint (#752) remains the interim; #591's **64** stat-less Vaults weapons still carry no augment slot while all **32** slottable worn items do (half A, #762); #769's `Slave Lord's Endurance` is stamped unresolved at both tiers, so one of the three sets the Slaver's station offers is off the table. Then **#764**, whose harvest-independent half was **settled on 2026-09-17 and came back negative** (PR #778): **39 of the 112** untyped Trinket effects did have a carrier tooltip on disk in `affix_tooltip.json` (#713), and a *named-item* carrier's tooltip is **not** admissible evidence for the *crafted* effect's bonus type. Tested where the answer was already known — of the 12 effects carrying both a wiki-stated crafted type and a tooltip, it agrees on 9 and contradicts on 3, and `Healing Amplification` contradicts on an **exact label match**, which forecloses trusting label-matched tooltips too. That is *no effect is typed by its own name* one step out, recorded by that PR in `docs/wiki-evidence/essence-crafting-bonus-types.md` with two guards. **So #764's scope is the full 112, not 73**, and all of it is wiki-gated along with the three unserved menus (Rune Arm **117** item-slots, Melee 3, Ring 3). Then **#765** (**147** random-roll item-slots, **103** of them the Drow Weapon Master weapons; every pool is an empty dict in the catalog, so one wiki harvest per family). **#767 was decided on 2026-09-17: a non-goal**, recorded in `AGENTS.md` and closed as not planned. The **42** Thunder-Forged blanks stay solvable as Orange augment hosts; only their tier recipes are unmodelled, no source carries them, and no player report has ever asked. The status row above now says so instead of showing a ❌ that read as a promise. **#196 was decided on 2026-09-17: a non-goal**, recorded in `AGENTS.md` and closed as not planned. It reverses that issue's own 2026-09-06 note, which argued for scheduling rather than declining; the reversal is named in the non-goals entry rather than left to look like an oversight. No source carries filigree data at all.  **Closed since the 2026-09-02 judgement:** every issue that line named — #664 (a re-read of the Concealment page: Master Illusionist is *stacking*, not an enhancement bonus, so 170 is what the wiki predicts and no code changed), #672 (PR #678), #459 (PR #686), #194 (PR #688), #529 (PR #711), #192 (PR #710), #331 (not planned) — plus the whole 2026-09-10 feedback batch (#740–#747), #766 (PR #768), the 2026-09-16 batch — #772 and #773 (PR #775), #774 (PR #776) — and #770 (PR #779, which also gave the re-shoot a one-command recipe and retired `annotate.py`) — which is why this line is re-judged rather than appended to. **Deliberately not filed:** multi-column priorities (#744 closed on the reasoning that a taller single column with pointer drag suffices; reopen on a report, not a sweep), and admitting `Enhancement bonus` / `Elemental Resistance` on the Cannith weapons (#591's body carries it, conditional on the dump landing). **Non-goals** (weighted-sum/Pareto modes, user accounts, exhaustive Green Steel space, numeric TWF penalty, attainability as a default solver input) are listed in `AGENTS.md` and are deliberately *not* issues.
