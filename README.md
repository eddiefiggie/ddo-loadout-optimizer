...
**Current build:** 08302026.10 — the live site's footer shows the deployed value. `tests/test_build_stamp.py` fails the build when this line drifts from `web/app.js`, so it cannot go stale silently.
...
>
> **Genuinely open:** [all open issues](https://github.com/eddiefiggie/ddo-loadout-optimizer/issues) — GitHub Issues is the single source of truth, so do not maintain a second list here. Self-deferred work carries the [`backlog`](https://github.com/eddiefiggie/ddo-loadout-optimizer/issues?q=is%3Aissue+is%3Aopen+label%3Abacklog) label; user-reported work is `bug`/`enhancement`. **Next up, in the order last judged (2026-08-28):** **#578** (a stat with no reachable source, ranked alone, produces a solve that never returns — a player-facing defect with no cancel control), then **#459** (name the picks a cap clamps out; unblocked by #199 on 2026-08-28, though #574 measured that the ceiling binds only through declared credits, so re-measure before building), then **#411** (~264 named-looking owned names matching nothing), then **#357** — the loadout library, whose plan is implementation-ready with all 42 requirements traced. **#199 CLOSED 2026-08-28** by PR #574, which taught the app the game's own stat ceilings and shipped four wiki-evidenced *refusals* as data so a missing cap reads as "the wiki states no ceiling" rather than "nobody looked". **#449's browser pass completed 2026-08-28** — seven behaviours verified live, and AE12 struck as unreachable because `canFindUpgrades()` is true for any solve that picked gear. **Non-goals** (weighted-sum/Pareto modes, user accounts, exhaustive Green Steel space, numeric TWF penalty, attainability as a default solver input) are listed in `AGENTS.md` and are deliberately *not* issues.
