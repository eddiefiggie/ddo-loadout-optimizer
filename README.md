...
**Current build:** 08202026.5 — the live site's footer shows the deployed value. `tests/test_build_stamp.py` fails the build when this line drifts from `web/app.js`, so it cannot go stale silently.
...
>
> **Genuinely open:** [all open issues](https://github.com/eddiefiggie/ddo-loadout-optimizer/issues) — GitHub Issues is the single source of truth, so do not maintain a second list here. Self-deferred work carries the [`backlog`](https://github.com/eddiefiggie/ddo-loadout-optimizer/issues?q=is%3Aissue+is%3Aopen+label%3Abacklog) label; user-reported work is `bug`/`enhancement`. **Next up, in the order last judged:** **#359** (owned-augment modelling — the owned-gear pool constrains base items only), then **#357** (the loadout-library / Compare plan, 42 requirements, never tracked). A further Utility batch is available whenever wanted: the encoding gate is proven clean to 32 and the cap is 28, so ~3 more names fit before the next measurement is needed. **#404, #402, #349, #396, #367, #370 and #380 all shipped 2026-08-20** (PRs #406, #405, #400, #399, #397, #395, #394). **#229** was re-scoped 2026-08-19: the population fell 183 → 23 names and its `Ki` example is resolved; 10 of the 23 are upstream parse fragments that must never be admitted, 13 are plausible enchantments needing rulings. **Non-goals** (weighted-sum/Pareto modes, user accounts, exhaustive Green Steel space, numeric TWF penalty, attainability as a default solver input) are listed in `AGENTS.md` and are deliberately *not* issues.
