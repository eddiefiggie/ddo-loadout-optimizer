...
### Umbrella stat
An affix that buffs a whole family at once, expanded into that family's concrete affixes so a single-member [[Target]] is credited — the optimizer matches a target only by exact stat name, so an unexpanded umbrella contributes nothing. Two families: **ability** ("All Ability Scores +15", "Well Rounded" → the six abilities) and **spell DC** ("Spell Focus Mastery", bare "Spell Focus" → the seven school focuses), the latter wiki-confirmed as applying "to the DC of all your spells". Expansion preserves value and bonus type, so the existing `name||equivType(type)` bucketing reproduces the real rule for free: same bonus type collapses to the highest, different types stack. Membership is an explicit allowlist requiring a wiki quote — `Rune Arm Focus` is excluded because the wiki says it "isn't directly tied to a Spell School", and **spell lore is not an umbrella at all** (universal and element lore genuinely stack; see `docs/wiki-evidence/spell-lore.md`). Expansion is bounded by the shape of its container, not by the channel it runs in: on an item the family members co-exist, but inside a select-one [[Roll group (choice-slot)|choice-slot]] they are alternatives, so expanding a flat one-affix-per-record pool converts one craft into N mutually exclusive crafts and the slot grants one member where the game grants the family. **Expand inside an option, never across options** — a multi-affix option record keeps the family atomic; a [[Dino insert]] was always safe for this reason and the Viktranium pool was not.
