...
// larger copy contributes and the block presented the dead one identically.
//
// #488 moved the credit out of the render layer into `bundleGroups`, so these
// assert the ANNOTATION and then that each surface reads it. The app-only version
// is what left every export listing a superseded bundle as live.
{
  const Pb = require("../web/projection.js");
  const Xb = require("../web/exporters.js");
  ...
  const credited = (src) => ({ bonus_type: "Enhancement", value: 12, source: src,
    sourceKind: "worn", slot: "Armor", hostIds: [src] });
  const EMPTY = { augmentsPlaced: [], setsActive: [], dinoPlaced: [], ncPlaced: [],
    rollPlaced: [], vikPlaced: [], sealPlaced: [], tfPlaced: [], gsPlaced: [],
    jokerPlaced: [], membershipPlaced: [] };
  ...
  const build = {
    ...
    breakdown: { Stunning: [credited("Chain Shirt of Toppling")],
                 Vertigo: [credited("Chain Shirt of Toppling")] },
    ...EMPTY,
  };
  const groupFor = (b, name) => Pb.bundleGroups(b).find((g) => g.carrier === name);

  test("#488: bundleGroups annotates the credited carrier live and the beaten one dead", () => {
    const live = groupFor(build, "Chain Shirt of Toppling");
    const dead = groupFor(build, "Bracers of Baphomet");
    assert.strictEqual(live.state, "live", "this copy is what the solver counted");
    assert.deepStrictEqual(live.members.map((m) => m.state), ["live", "live"]);
    assert.strictEqual(dead.state, "superseded", "a larger source fills the bucket, so this copy adds nothing");
    assert.deepStrictEqual(dead.members.map((m) => m.state), ["superseded", "superseded"]);
  });

  test("#488: within ONE carrier, superseded members are judged apart from live ones", () => {
    ...
    const solo = {
      chosen: [{ slot: "Bracers", variant: { variant_id: "Legendary Bracers of Baphomet", ml: 35,
        affixes: [bundleAffix("Stunning", 6), bundleAffix("Vertigo", 6),
                  bundleAffix("Stunning", 12), bundleAffix("Vertigo", 12)] } }],
      effective: { Stunning: 12, Vertigo: 12 },
      breakdown: { Stunning: [credited("Legendary Bracers of Baphomet")],
                   Vertigo: [credited("Legendary Bracers of Baphomet")] },
      ...EMPTY,
    };
    const g = groupFor(solo, "Legendary Bracers of Baphomet");
    assert.deepStrictEqual(g.members.map((m) => m.state),
      ["superseded", "superseded", "live", "live"], "the 6s are dead against the item's own 12s");
    assert.strictEqual(g.state, "live", "the bundle itself is live — some of it is credited");
  });

  test("#488: with no breakdown to judge against, NOTHING is claimed either way", () => {
    ...
    // about a build the code could not see. Inventing the fact is worse than the
    // duplicate the guard exists to remove.
    const bare = { ...build, breakdown: {} };
    const g = groupFor(bare, "Bracers of Baphomet");
    assert.strictEqual(g.state, null, "unjudged, not dead");
    assert.deepStrictEqual(g.members.map((m) => m.state), [null, null]);
    const html = R.bundlesBlock(bare, new Map());
    assert.ok(!/is-incidental/.test(html), "no card is asserted superseded");
    assert.ok(!/already fills these bonus-type buckets/.test(html), "and nothing is claimed");
    assert.ok(!/bundle-dead/.test(html), "no member is struck through");
    const md = Xb.toMarkdown({ name: "B", inputs: { ml: 34, pool: "all", priorities: [] }, snapshot: bare });
    assert.ok(!/superseded/.test(md), "the share claims nothing either");
  });

  test("#488: an UNRANKED bundle is not called superseded — nothing is filling its bucket", () => {
    // Measured on six real fixtures, a two-state version called 16 of 20 bundles
    // superseded; most were merely unranked (`Parrying` on Scales of the Exile,
    // `Speed` on Gloves of the Strong). The export would have told the player a
    // larger source was winning a bucket nothing was competing for. Splitting the
    // state took it to 4 live / 7 superseded / 9 unranked.
    const unranked = {
      chosen: [carrier("Gloves of the Strong", 8)],
      effective: { Doublestrike: 20 },
      // Doublestrike is the ranked priority; the bundle grants Stunning/Vertigo,
      // which the player never asked for.
      breakdown: { Doublestrike: [{ bonus_type: "Quality", value: 20, source: "Elsewhere",
        sourceKind: "worn", slot: "Ring", hostIds: ["Elsewhere"] }] },
      ...EMPTY,
    };
    const g = groupFor(unranked, "Gloves of the Strong");
    assert.strictEqual(g.state, "unranked", "not on the priority list, so not superseded");
    assert.deepStrictEqual(g.members.map((m) => m.state), ["unranked", "unranked"]);
    const html = R.bundlesBlock(unranked, new Map());
    assert.ok(!/bundle-dead/.test(html), "no member is struck through");
    assert.ok(!/already fills these bonus-type buckets/.test(html),
      "and no competitor is claimed for a bucket nothing contests");
    const md = Xb.toMarkdown({ name: "U", inputs: { ml: 34, pool: "all", priorities: ["Doublestrike"] }, snapshot: unranked });
    assert.ok(/Combat Mastery/.test(md), "the share still lists the bundle");
    assert.ok(!/superseded/.test(md), "…without the qualifier");
  });

  test("#485: the app keeps the superseded card and states why", () => {
    const html = R.bundlesBlock(build, new Map());
    ...
    assert.ok(/already fills these bonus-type buckets/.test(html), "…and says so in words, not only a shade");
    assert.ok(/bundle-dead/.test(html), "the dead members are marked individually");
  });

  test("#488: every sets-rendering export carries the superseded qualifier", () => {
    const rec = { name: "B", inputs: { ml: 34, pool: "all", priorities: ["Stunning", "Vertigo"] }, snapshot: build };
    const outs = { markdown: Xb.toMarkdown(rec), bbcode: Xb.toBBCode(rec),
                   csv: Xb.toCsv(rec), html: Xb.toPrintHtml(rec),
                   gearset: Xb.toGearset(rec) };
    for (const [fmt, out] of Object.entries(outs)) {
      assert.ok(/Combat Mastery/.test(out), `${fmt}: still lists the bundle`);
      assert.ok(/superseded/.test(out),
        `${fmt}: names the beaten copy — a share that omitted this says the opposite of the screen it came from`);
    }
    // …and only ONCE: the live carrier's copy must not be qualified.
    assert.strictEqual((outs.markdown.match(/superseded/g) || []).length, 1,
      "the credited carrier is not marked superseded");
  });
}
