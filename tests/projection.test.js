...
// ---- #239 U2: free slots ---------------------------------------------------
//
// A free slot is one the ranked priorities did not constrain — every candidate
// tied, so a tie-break filled it. `whyThis` already identifies exactly this: its
// contract is "empty for a filler/tie-break pick". Reuse it rather than writing
// a second notion of "contributes nothing". A slot feeding a satisfied set is
// excluded even when its own affixes score zero — calling a set piece dead
// weight reads as a bug, not as honesty.

function freeSlotBuild() {
  return {
    status: "optimal",
    chosen: [
      { slot: "Goggles", variant: { variant_id: "Wins", affixes: [{ name: "Deadly", type: "Insightful", value: 9 }] } },
      { slot: "Ring", variant: { variant_id: "Filler", affixes: [{ name: "Unranked", type: "Equipment", value: 5 }] } },
      { slot: "Boots", variant: { variant_id: "Filler2", affixes: [] } },
    ],
    breakdown: { Deadly: [{ bonus_type: "Insightful", value: 9, source: "Wins", sourceKind: "item",
      slot: "Goggles", hostIds: ["Wins"] }] },
    augmentsPlaced: [], setAugmentsPlaced: [], membershipPlaced: [], setsActive: [],
  };
}

test("U2/#239: slots winning no ranked target are counted free", () => {
  const f = P.freeSlots(freeSlotBuild());
  assert.strictEqual(f.count, 2, "Ring and Boots win nothing; Goggles wins Deadly");
  assert.deepStrictEqual(f.slots.slice().sort(), ["Boots", "Ring"]);
});

test("U2/#239: a loadout where every slot wins something has no free slots", () => {
  const build = freeSlotBuild();
  build.chosen = [build.chosen[0]];
  const f = P.freeSlots(build);
  assert.strictEqual(f.count, 0);
  assert.deepStrictEqual(f.slots, []);
});

test("U2/#239: a slot feeding a satisfied set is not free, though it wins nothing", () => {
  const build = freeSlotBuild();
  build.chosen[1].variant.set_bonus = [{ set: "Biting Sands" }];
  build.chosen[2].variant.set_bonus = [{ set: "Biting Sands" }];
  build.setsActive = ["Biting Sands"];
  const f = P.freeSlots(build);
  assert.strictEqual(f.count, 0,
    "both Ring and Boots complete the set — its bonus is unranked, but they are doing work");
});

test("U2/#239: a slot winning a ranked target only via a set bonus is not free", () => {
  const build = freeSlotBuild();
  build.breakdown.Deadly.push({ bonus_type: "Artifact", value: 4, source: "Biting Sands",
    sourceKind: "set", setYieldingSlots: ["Ring"], hostIds: ["Filler"] });
  const f = P.freeSlots(build);
  assert.ok(!f.slots.includes("Ring"), "whyThis already credits it through the set");
});

test("U2/#239: a missing or non-optimal result counts zero rather than throwing", () => {
  assert.strictEqual(P.freeSlots(null).count, 0);
  assert.strictEqual(P.freeSlots({ status: "infeasible" }).count, 0);
  assert.strictEqual(P.freeSlots({ status: "optimal", chosen: [] }).count, 0);
});
