...
test("the 161 unstamped boolean-composite components are ineligible", () => {
  ...
  // 837 not 589 since #746: +248, and the delta decomposes exactly into the two
  // composites that ruling shipped, with nothing else moving. `Dusk` -> Concealment
  // is 24 carriers x 1 = 24; `Ghostly` -> Hide + Move Silently is 112 carriers x 2 =
  // 224. `Enhanced Ghostly` carries the same two components but has ZERO item
  // carriers (it is a crafting-option name only), so it contributes nothing here —
  // stated because its absence from the arithmetic otherwise reads as an omission.
  // No Dusk carrier also carries `Blurry` or `Lesser Displacement`, so none of the
...
test("the classified population is 20,578 eligible of 42,185", () => {
  ...
  // #546 moved every number here, and each moved for a derived reason rather
  // than "the build says so". Measured as an A/B against a build with the
  // Riposte split disabled:
  //
  //   total    42,088 -> 42,185 (+97). The split emits three saves per covered
  //     record and renames the folded affix into the Armor Class it already
  //     counted as, so a rename is count-neutral. 93 saves added on the item
  //     channel + 6 on the augment channel, minus the 2 folded affixes DROPPED
  //     rather than renamed (`primary_suppressed` — those records already
  //     carried an Insight Armor Class, so renaming would have duplicated it).
  //     93 + 6 - 2 = 97. It is not 3x35: the anti-shadow rule withholds a save
  //     the record already carries in the same stacking bucket.
  //
  //   eligible 20,613 -> 20,578 (-35). Exactly the 35 folded `Riposte` affixes.
  //     They were engraved and overridable; what replaced them is generated by
  //     the split, and generated affixes are not offered for override. So the
  //     eligible population LOSES 35 while the total gains 97 — the two moving
  //     in opposite directions is the expected shape of an expansion, not a
  //     contradiction.
  //
