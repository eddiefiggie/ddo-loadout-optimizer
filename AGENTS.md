...
- **Any deploy that changes player-facing behavior bumps three things together:** the `?v=` cache-busts in `web/index.html` (Pages has no content hashing, so a stale one keeps serving cached code), the footer `BUILD` in `web/app.js`, and the `**Current build:**` line in `README.md`. The trigger is behavior, not file location: `web/` code changes AND solver-affecting data/seed/pipeline changes both qualify — the dataset is fetched `no-cache`, so a data-only merge changes every solve on the live site the moment it deploys, and the footer is the only version signal a player has. Doc-only changes (`docs/`, `CONCEPTS.md`, `AGENTS.md`) are exempt. `tests/test_build_stamp.py` fails the build when the three disagree — the rule predates the guard, was skipped by four PRs that left the footer under-reporting for two days, and its old `web/`-only trigger let the 2026-08-13 data batch (PR #294) change every caster solve while the footer claimed nothing happened.
